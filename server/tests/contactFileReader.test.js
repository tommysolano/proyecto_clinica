...
test('XLSX con celda de HORA: mapeada como variable {{hora}} queda en "HH:MM"', async () => {
  // Reproduce el bug real: Excel guarda "08:00" como fracción de día y el lector en
  // STREAMING de ExcelJS la entrega como número crudo (1,3333…) — perdiendo el
  // numFmt, así que no se puede distinguir de un decimal cualquiera en el lector.
  // La normalización segura vive en el MAPPER: una columna "Hora" → custom:hora se
  // deja en "HH:MM"; un decimal de otra columna NO se toca.
  const ExcelJS = require('exceljs');
  const { mapRow } = require('../utils/contactRowMapper');
  const wb = new ExcelJS.Workbook();
  const ws = wb.addWorksheet('C');
  ws.addRow(['Nombre', 'Telefono', 'Hora', 'Precio']);
  const r1 = ws.addRow(['Ana', '0999111222', new Date(Date.UTC(1899, 11, 31, 8, 0, 0)), 0.15]);
  r1.getCell(3).numFmt = 'hh:mm';
  const r2 = ws.addRow(['Beto', '0988776655', new Date(Date.UTC(1899, 11, 31, 14, 30, 0)), 1.5]);
  ...
  const mapping = [
    { column: 'Telefono', field: 'phone' },
    { column: 'Hora', field: 'custom:hora' },
    { column: 'Precio', field: 'custom:precio' },
  ];
  const a = mapRow(rows[0], mapping);
  const b = mapRow(rows[1], mapping);
  assert.equal(a.contact.customFields.hora, '08:00', `de "${rows[0].Hora}" → 08:00`);
  assert.equal(b.contact.customFields.hora, '14:30', `de "${rows[1].Hora}" → 14:30`);
  // El decimal de otra columna NO se convierte a hora: se guarda tal cual.
  assert.equal(a.contact.customFields.precio, '0.15', 'un decimal cualquiera no se toca');
  fs.unlinkSync(file);
});
