...
test('sendMessage con replyTo guarda el snapshot del mensaje citado y el remitente', async () => {
  ...
  // La petición devuelve el mensaje ya guardado (estado 'queued'); la entrega va
  // por detrás y el proveedor stub, con el token vacío, acabará rechazándola. Lo
  // que se comprueba aquí es que el mensaje persiste con SU CITA, pase lo que
  // pase con el proveedor.
  assert.equal(out.statusCode, 201, JSON.stringify(out.payload));
  assert.equal(out.payload.deliveryStatus, 'queued', 'nunca se responde "enviado" antes de tiempo');
...
test('replyTo de otra conversación se ignora (no se cita un mensaje ajeno)', async () => {
  ...
  const out = await H.runController(chat.sendMessage, req);
  // El envío se acepta y se entrega por detrás (el proveedor stub lo rechazará);
  // lo que importa aquí es que el mensaje NO se quede con una cita ajena.
  assert.equal(out.statusCode, 201, JSON.stringify(out.payload));
  const saved = await Message.findById(out.payload._id);
