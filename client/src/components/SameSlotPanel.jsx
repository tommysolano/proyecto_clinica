...
export default function SameSlotPanel({ date, startTime, excludeId, clinicId, compact = false }) {
  ...
  const wrapperClass = compact
    ...
    : 'border-l border-slate-200 lg:pl-5';

  return (
    <Wrapper className={wrapperClass}>
      <div className={compact ? 'space-y-2' : 'sticky top-0 space-y-3'}>
        <div>
        ...
        <div
          className={`rounded-xl border border-slate-200 bg-slate-50/40 p-3 overflow-y-auto ${
            compact ? 'min-h-[120px] max-h-[220px]' : 'min-h-[200px] max-h-[420px]'
          }`}
        >
          {!date || !startTime ? (
