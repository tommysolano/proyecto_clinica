  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-slate-800 flex items-center gap-2"><HiOutlineBanknotes className="text-emerald-600" /> Préstamos a Empleados</h1>
      ...
      <Modal isOpen={show} onClose={() => setShow(false)} title="Nuevo préstamo">
        <form onSubmit={submit} className="space-y-3">
          <select required value={form.employee} onChange={(e) => setForm({ ...form, employee: e.target.value })} className="w-full border border-slate-200 rounded-lg px-3 py-2">
            <option value="">Empleado...</option>
            {employees.map((e) => <option key={e._id} value={e._id}>{e.firstName} {e.lastName}</option>)}
          </select>
          <div className="grid grid-cols-2 gap-3">
            <select value={form.type} onChange={(e) => setForm({ ...form, type: e.target.value })} className="border border-slate-200 rounded-lg px-3 py-2"><option>EMPRESA</option><option>QUIROGRAFARIO</option><option>HIPOTECARIO</option></select>
            ...
            <input type="number" step="0.01" required placeholder="Capital" value={form.principal} onChange={(e) => setForm({ ...form, principal: +e.target.value })} className="border border-slate-200 rounded-lg px-3 py-2" />
            <input type="number" required placeholder="Cuotas" value={form.installmentsCount} onChange={(e) => setForm({ ...form, installmentsCount: +e.target.value })} className="border border-slate-200 rounded-lg px-3 py-2" />
          </div>
